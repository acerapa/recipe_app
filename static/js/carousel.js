class Carousel
{
  ...
  createCarousel() {
    // create styling
    this.generateStylings()
    
    // get carousel container
    this.carouselContainer = document.getElementById(this.carouselId)
    if(this.validateCarouselContainer(this.carouselContainer)) {
      let carouselItems = document.getElementsByClassName('carousel-item')
      this.generateRedirectionDots(this.carouselContainer, carouselItems)
      this.generatePerviousNextButtons(this.carouselContainer)
      
      // check if auto carousel auto slide
      if (this.options.isAutoSwitch) {
        let interval = setInterval(() => {
          if (this.options.dataId > document.getElementsByClassName('carousel-dots').length) {
            this.options.dataId = 1
          }
          this.carouselPagesSwitcher(this.options.dataId)
          this.options.dataId += 1
        }, 5000)
      }
    }
   ...
   */
  validateCarouselContainer(carouselContainer) {
    let isValid = true
  ...
  generateRedirectionDots(carouselContainer, carouselItems) {
    ...
    Array.from(carouselItems).forEach(element => {
      ...
      carouselDot.addEventListener('click', this.carouselPagesSwitcher)
  ...
  carouselPagesSwitcher(dataId) {
    // identify dataId value (element or method)
    dataId = dataId.target ? this.id : dataId
    // remove active class
